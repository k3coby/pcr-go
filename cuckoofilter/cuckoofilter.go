...
func (cf *CFilter) Filter2Bytes() [][]byte {
	...
	bytes := make([][]byte, cf.numBuckets * BUCKET_SIZE)
...
// This function returns a SHA224 hash output as the fingerprint of
// the given input element under a different seed
func GetFP2(element []byte) []byte {

	h := sha256.New224()
	h.Write(append(element, []byte(strconv.Itoa(FP_SEED2))...))
	fp := h.Sum(nil)
	return fp
}
...
func hashFunction(input []byte, numBuckets uint) uint {
	...
	newHash := fnv.New32a()
	_, err := newHash.Write([]byte(input))
	if err != nil {
		panic(err)
	}
